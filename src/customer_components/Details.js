...
export default function Details() {
    ...
    const add_product_to_bag=()=>{
        ...
        axios.post(url, fData).then(function(response){
            data=response.data
            if(data){
                toast.success(`${location.state.product.value.name} added successfully to your bag`)
            }
            else{
                toast.info(`${location.state.product.value.name} are already exists in your bag`)
            }
            ...
            <div style={{marginTop:"10rem",marginLeft:"30px"}} className='detail_container'>
                ...
                <div className="product_details">
                        ...
                        <div style={{display: 'flex',alignItems: 'center'}}>
                            <p style={{fontFamily:'Roboto,sans-serif',display: 'inline-block',fontSize:"1.5rem"}}>Rating:</p>
                            <p style={{fontFamily:'Roboto,sans-serif',display: 'inline-block'}}><ShowRating rating={location?.state?.product.value.Rating} size={25}/></p>
                        </div>
                        <div>
